...
- **Papers to read:**
  ...
  - [Stratified Hazard Sampling](https://arxiv.org/abs/2601.02799) (2026-09-01 audit, #1) — training-free, hyperparameter-free replacement for independent-Bernoulli unmasking; direct fix for the under-/over-editing variance mechanism.
  - [ReCAP](https://arxiv.org/abs/2505.19089), [code](https://github.com/liebenxj/ReCAP) (2026-09-01 audit, #2) — caches decoded-context feature embeddings across refinement steps; cheapest available decoding-speed win to try first.
  - [Discrete Adjoint Matching](https://arxiv.org/abs/2602.07132) (2026-09-01 audit, #3) — reward fine-tuning of CTMC generators that provably converges to the reward-tilted distribution; candidate replacement for GRPO/DPO-style dLLM alignment.
- **People / labs to track (seeded 2026-08-01):**
  ...
  - **(added 2026-09-01)** Oswin So / Chuchu Fan (MIT) + Meta FAIR (Ricky Chen, Guan-Horng Liu) discrete adjoint-matching line — [arXiv 2602.07132](https://arxiv.org/abs/2602.07132), [ICLR 2026 poster](https://iclr.cc/virtual/2026/poster/10009131); publishes under control-theory keywords, invisible to a cs.CL filter, directly relevant to dLLM RL alignment.
- **Open questions:**
  ...
  - Does stratified-phase event scheduling (SHS) reduce under-/over-editing failures at fixed NFE compared to independent-Bernoulli unmasking? (from finding #1, 2026-09-01 audit)
  - Do trained MDLM-style (mean-parameterized) checkpoints exhibit the same ~25% Bayes-realizability violation rate that M2S found in SEDD checkpoints, or is it SEDD-specific? (from finding #4, 2026-09-01 audit)
...
Anchor: **SEDD** (D3PM and MDLM were mined last month; SEDD's citer set skews
toward critique/foundations clusters the other two don't pull in). Move 1 was
search-attested only — no programmatic citation-graph pull (Semantic Scholar
reports 267 citations / 74 "highly influential" but the sub-counts don't
reconcile) — so cluster sizes below are qualitative, not counted.

**Ranked findings (most likely to change how you approach your own work, first):**

1. **Stratified Hazard Sampling (SHS)** — frames per-token edits as a counting process (cumulative hazard for CTMC / cumulative jump mass for DTMC) and places events by stratifying that quantity with one random phase per position, instead of independent per-step Bernoulli draws. Independent Bernoulli decisions induce Poisson-binomial variance in per-position jump counts — the likely mechanism behind under-/over-editing. Training-free, hyperparameter-free, drop-in; testable tonight on any dLLM's unmasking loop. [arXiv 2601.02799](https://arxiv.org/abs/2601.02799) — preprint (v2 Feb 2026)
2. **ReCAP** (vision, UCLA StarAI+PKU) — interleaves full bidirectional evaluations with cheap steps that cache and reuse *feature embeddings of already-decoded context tokens* across refinement steps (distinct from within-step KV-caching). Up to 2.4x faster on MaskGIT/MAGE/MAR at minimal quality loss. Cheapest axis to try against an existing dLLM decoder before touching the sampler. [arXiv 2505.19089](https://arxiv.org/abs/2505.19089), [code](https://github.com/liebenxj/ReCAP) — peer-reviewed. Complement: [Fixed-Point Masked Generative Modeling (EPFL), arXiv 2605.31215](https://arxiv.org/abs/2605.31215) — "three-state" (unchanged/still-masked/newly-revealed) fixed-point warm-start, architecturally distinct from the fixed-point distillation already on the Watchlist.
3. **Discrete Adjoint Matching (DAM)** — discrete-state port of Adjoint Matching for reward fine-tuning of CTMC generators; provably converges to the reward-tilted model distribution rather than a mode-collapsed one. A principled alternative to GRPO/DPO-style dLLM alignment loops — testable by comparing KL-to-base at matched reward. [arXiv 2602.07132](https://arxiv.org/abs/2602.07132), [ICLR 2026 poster](https://iclr.cc/virtual/2026/poster/10009131) — top-tier. Authors are control/robotics (Oswin So, Chuchu Fan, MIT) + Meta FAIR (Ricky Chen, Guan-Horng Liu); indexed as control theory, invisible to a cs.CL filter.
4. **Mean-to-Score (M2S)** — direct SEDD critique: the learned score ratios are not Bayes-realizable from any clean-token posterior ~25% of the time in trained checkpoints, producing negative pre-normalization weights during sampling. Fix is a reparameterization (predict the posterior mean, map to score via an exact linear map), not a new sampler — GenPPL 203.6 -> 175.1 on the same architecture. Actionable as a checkpoint audit before blaming few-step degradation on the sampler. [arXiv 2607.21372](https://arxiv.org/abs/2607.21372) — preprint
5. **Discrete Feynman-Kac Correctors** — training-free SMC at inference time for (a) tempering, (b) sampling the *product of marginals* of several differently-conditioned diffusion processes, (c) marginal x external reward. No fine-tuning. Gives a product-of-experts decoding primitive (e.g. combine instruction + style conditioning as a product of marginals at sample time) distinct from the single-tilted-posterior amortized twisted SMC already on the Watchlist. [arXiv 2601.10403](https://arxiv.org/abs/2601.10403) (Bengio, Aspuru-Guzik, Neklyudov) — preprint
6. **PhysNovo** (de novo peptide sequencing) — embeds a knapsack-based feasibility kernel directly in the diffusion transition to enforce exact precursor-mass consistency, resolved iteratively via bidirectional evidence. Transferable as a template for enforcing an exact combinatorial global constraint (length budget, bracket balance, JSON schema) by intersecting the unmasking kernel with a DP-computed feasible set instead of rejection sampling / post-hoc repair. [ICML 2026 poster](https://icml.cc/virtual/2026/poster/61142) — top-tier venue, paper text itself unverified beyond the abstract (see couldn't-verify). Sibling: [Diffusion Decoding for Peptide De Novo Sequencing, arXiv 2507.10955](https://arxiv.org/abs/2507.10955).
7. **Type-aware forward processes** (CAD/geometric program synthesis) — cascaded two-stage diffusion (commands then parameters) using a *different corruption kernel per attribute type*: absorbing-state for commands, Gaussian for coordinate continuity, scale-invariant for dimensional values, prior-preserving for booleans. Text vocab is not homogeneous either — numerals, identifiers, tool-call arguments could each get a type-specific transition matrix instead of one absorbing mask. [arXiv 2605.05031](https://arxiv.org/abs/2605.05031) — preprint
8. **Beam search over unmasking orders** (LLaDA-Rec, recommender systems) — adapts beam search for adaptive-order discrete diffusion by maintaining a beam over *unmasking trajectories* rather than token prefixes, orthogonal to remasking-corrector heuristics. [arXiv 2511.06254](https://arxiv.org/abs/2511.06254) — preprint. Companion: [MaskGR, arXiv 2511.23021](https://arxiv.org/abs/2511.23021).
9. **Discrete Adjoint Schrödinger Bridge Sampler** — unifies adjoint matching and ASBS in discrete state spaces; the adjoint-matching mechanism itself is state-space agnostic but this extension requires a **cyclic group structure on the state space** — a concrete design constraint on how a vocabulary is indexed, not cosmetic. [arXiv 2602.08243](https://arxiv.org/abs/2602.08243), [OpenReview](https://openreview.net/forum?id=6gVec6LNtu) — preprint, venue unconfirmed. Upstream: [MDNS, NeurIPS 2025](https://arxiv.org/abs/2508.10684).
10. **The Score Hamiltonian** — exact correspondence between score-based sampling and adiabatic transport of ground states of a Schrödinger operator built from the learned score; the fundamental few-step sampling limit is (squared score-matching error) / (spectral gap of the Score Hamiltonian) = inverse Poincaré constant of the data density. Reframes noise-schedule tuning as an eigenvalue-estimation problem. Continuous-state only; porting the spectral-gap diagnostic to CTMC/absorbing-mask models is open. [arXiv 2606.05217](https://arxiv.org/abs/2606.05217) — preprint (math-ph), longer-horizon bet
11. **FlexFlow** (DNA + peptides, Aalto) — variable-length generation via edit operations conditioned on a shared *global* latent, with Dirichlet-controlled operation mixing. A different length/insert-delete control knob than the per-position-rate Edit Flows line already on the Watchlist — flagged as adjacent, not fully distinct. [arXiv 2606.10543](https://arxiv.org/abs/2606.10543) — preprint
12. **Minimax optimality of SEDD** — minimax lower bound under score-entropy loss plus an MLE-based thresholding estimator matching it up to polylog factors; SEDD is near-optimal in KL only given specific initialization/discretization conditions — i.e. this names which hyperparameters are statistically load-bearing vs. cosmetic. [arXiv 2608.20635](https://arxiv.org/abs/2608.20635) (21 Aug 2026) — preprint, author affiliations unverified
13. **Energy-based generation via strictly proper scoring rules** (neuroscience, EAG) — trains an energy-based head under a strictly proper scoring rule to model *joint* dependence in one step rather than independent per-token marginals, beating diffusion baselines on efficiency at matched realism on spiking data. The scout's own analogy (not the paper's claim) is that this targets the factorization error behind parallel-decoding artifacts in text dLLMs — unverified transfer. [arXiv 2511.17606](https://arxiv.org/abs/2511.17606), [AAAI 2026](https://ojs.aaai.org/index.php/AAAI/article/view/36992) — peer-reviewed, but the text-diffusion connection is speculative
14. **Constrained Discrete Diffusion via Lagrangian-dual projection** — differentiable per-step projection solving a Lagrangian dual program, reporting zero constraint violations while preserving validity/novelty (with a molecular variant adding synthetic-feasibility/originality projections). Ranked last because it has a cs.CL-facing framing and may already be in your feed. [NeurIPS 2025](https://papers.nips.cc/paper_files/paper/2025/file/124c1cc9f7750f512ed81cdbf2fd5bf5-Paper-Conference.pdf), [arXiv 2503.09790](https://arxiv.org/abs/2503.09790) — top-tier venue
15. **Deterministic Discrete Denoising** (nonlinear dynamics, Osaka) — replaces stochastic reverse transitions with a deterministic reverse chain driven by weakly-chaotic herding dynamics; no retraining, no continuous embeddings; reported gains on text, images, and diffusion-based combinatorial optimization — the discrete analogue of DDIM. Low-ranked pending verification (see couldn't-verify: author list and acceptance status both unconfirmed). [arXiv 2509.20896](https://arxiv.org/abs/2509.20896), [OpenReview](https://openreview.net/forum?id=CHtLFyDbZp) — preprint / under review

**5 unwatched venues/labs/communities (one verified result each; none overlap the six named in the 2026-08-01 audit):**

- Generative recommendation / semantic-ID community (cs.IR, RecSys/SIGIR) — [LLaDA-Rec, arXiv 2511.06254](https://arxiv.org/abs/2511.06254), adaptive-order beam search for masked diffusion decoding
- Computational proteomics / de novo peptide sequencing (ICML AI4Science-adjacent, Casanovo/InstaNovo lineage) — [PhysNovo, ICML 2026 poster](https://icml.cc/virtual/2026/poster/61142), knapsack feasibility kernel for exact mass-constrained discrete diffusion
- Georgia Tech applied-math sampling group (Tao, Chen, Guo, Zhu, Choi) x Meta FAIR (Liu, Chen) discrete stochastic-optimal-control axis — [Discrete Adjoint Schrödinger Bridge Sampler, arXiv 2602.08243](https://arxiv.org/abs/2602.08243); upstream [MDNS, NeurIPS 2025](https://neurips.cc/virtual/2025/loc/san-diego/poster/115255)
- Nonlinear-dynamics / herding derandomized-sampling community (Hideyuki Suzuki, Osaka University) — [Deterministic Discrete Denoising, arXiv 2509.20896](https://arxiv.org/abs/2509.20896)
- UCLA StarAI + PKU tractable-inference group (Van den Broeck, Liu, Liang) — [ReCAP, arXiv 2505.19089](https://arxiv.org/abs/2505.19089), [lab publications](http://starai.cs.ucla.edu/publications/)
- (bonus, weaker) CAD/geometric program synthesis community, systematically designing per-attribute-type transition kernels — [arXiv 2605.05031](https://arxiv.org/abs/2605.05031)

**Couldn't verify / flagged by the scout as its own inference, not sourced:**
- SEDD citer census (Move 1) — no programmatic citation-graph pull; Semantic Scholar's 267-citation / 74-highly-influential figures don't reconcile with per-category sub-counts. Cluster table is qualitative/search-attested only.
- "Discrete Stochastic Localization for Non-autoregressive Generation" — two conflicting arXiv IDs under the same title (2605.12836 vs 2602.16169); could not determine which is canonical or whether these are distinct papers. Excluded from the ranked list.
- Deterministic Discrete Denoising (#15) — author list differs across sources; "submitted to ICLR 2026" acceptance not confirmed; v3 date (29 Jan 2026) unconfirmed.
- Discrete Adjoint Schrödinger Bridge Sampler (#9) — venue conflicting (ICML 2026 vs. ICLR 2026 DeLTa Workshop poster across sources); treat as preprint.
- Score Hamiltonian (#10) — submission-date conflict (May 28 2026 reported vs. arXiv ID implying June 2026).
- Minimax Optimality of SEDD (#12) — author affiliations unverified.
- PhysNovo (#6) — ICML poster page confirmed, but full paper/proceedings text not independently verified beyond the abstract.
- FlexFlow (#11) — Aalto affiliation and comparative claims are abstract-snippet-only.
- Scout's own unverified inferences: ReCAP's context-feature reuse being orthogonal to KV-caching in dLLM decoders (analogy, not tested); M2S's ~25% Bayes-realizability violation rate transferring to mean-parameterized (MDLM-style) models — the paper measures SEDD checkpoints specifically; EAG's strictly-proper-scoring-rule head addressing text dLLM factorization error — EAG operates on autoregressive-in-latent-time spike data, so this is a proposed analogy, not a result.
- Materials/crystals: could not confirm any cluster that specifically cites SEDD (GenLCS, Materium use mask-and-replace / AR token schemes instead); excluded.
- Speech: found no genuinely new (i.e., not already on the Watchlist) speech-side decoding trick this month. [Mask, Sample, Revise, arXiv 2606.13989](https://arxiv.org/abs/2606.13989) (SC-ReMask) is likely redundant with the remasking/corrector line already tracked.
